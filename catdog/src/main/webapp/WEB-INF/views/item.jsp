...
<div id="wrap">
	...
	<div id="items">
		<div class="d-flex justify-content-between" style="margin:40px 0 20px;">
			<div>
				<span style="font-size:1.5em;">${category}</span>&nbsp;&nbsp;
				<span class="badge badge-secondary">${itemCount}</span>
			</div>
			<div>
				<select id="sort" name="sort" onchange="item_sort()">
					<option value="P_CODE">신상품</option>
					<option value="P_BUY">판매량</option>
					<option value="P_A_PRICE">높은가격</option>
					<option value="P_A_PRICE_ASC">낮은가격</option>
...
$(document).ready(function(){
	var sort = "${sort}";
	if(sort == "P_CODE")
		$("#sort").val("P_CODE").prop("selected", true);
	else if(sort == "P_BUY")
		$("#sort").val("P_BUY").prop("selected", true);
	else if(sort == "P_A_PRICE")
		$("#sort").val("P_A_PRICE").prop("selected", true);
	else if(sort == "P_A_PRICE_ASC")
		$("#sort").val("P_A_PRICE_ASC").prop("selected", true);
	else if(sort == "P_NAME")
		$("#sort").val("P_NAME").prop("selected", true);
});
...
function item_sort() {
	...
	if(category == "의류/하네스")
		...
		category = "cat";
	else if(category == "이동/산책")
		category = "walk";
	else if(category == "사료/간식")
		category = "food";
	else if(category == "강아지")
		category = "dog";
	
	var sort = $("#sort").val();
	
	location.href="item?category=" + category + "&sort=" + sort;
}
